<template>
    <form @submit.prevent="submitData">
        ...
        >
            <div class="mb-2 border-b pb-2">Component</div>
            <div class="grid grid-cols-6 gap-6">
                <div class="col-span-6 sm:col-span-4">
                    ...
                    <TextInput
                        ...
                        @input="transformSlugComponent(form.title)"
                    />
                ...
                <div class="col-span-6 sm:col-span-4">
                    ...
                    <TextInput
                        ...
                        type="text"
                        class="mt-1 block w-full"
                        @change="transformSlugComponent(form.slug)"
                    />
        ...
        >
            ...
            >
                {{ updateId !== null ? 'Update' : 'Create' }} Component
            </PrimaryButton>
        </div>
    </form>
</template>
...
const submitData = () => {
    if (updateId.value !== null) {
        axios
            .put(route('components.update', updateId.value), form.data())
            .then(() => {})
            .catch((err) => {
    ...
    axios
        .post(route('components.store'), form.data())
        .then(() => {
            router.visit(route('components.index'))
        })
        .catch((err) => {
            if (err.response.status === 422) {
                form.errors = err.response.data.errors
...
const transformSlugComponent = (value) => {
