...
const submitData = () => {
    if (updateId.value !== null) {
        axios
            .put(
                route('content.sections.posts.update', {
    ...
    axios
        .post(
            route('content.sections.posts.store', {
                sectionId: props.section.id
            }),
            form.data()
        )
        .then(() => {
            router.visit(route('content.sections.posts.index'), {
                sectionId: props.section.id
            })
        })
        .catch((err) => {
...
<template>
    <form @submit.prevent="submitData">
        ...
        >
            <div class="mb-2 border-b pb-2">Content Post</div>
